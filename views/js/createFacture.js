...
document.getElementById('createFactureForm').addEventListener("submit", function(event) {
  ...
  Swal.fire({
    ...
    if (result.value == true) {
      document.getElementById('createFactureForm').submit();
    } else {
      return false;
    }
  })
...
add.addEventListener("click", function() {
  if (prestation.value) {
    if (!document.getElementById(JSON.parse(prestation.value)._id)) {
      ...
      $.notify({
        icon: "ni ni-bell-55",
        message: "La prestation est déjà dans la facture."
      }, {
        type: 'danger',
        timer: 10,
        placement: {
